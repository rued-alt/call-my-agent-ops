...
function EvalPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: t.color.background }}>
      <OpsChrome t={t} staff={staff} agentHealth="healthy" evalQueueCount={7} />
      <main style={{ padding: `${t.space.unit * 8}px ${t.space.unit * 5}px`, fontFamily: t.type.bodyFamily, color: t.color.foreground }}>
        <h1 style={{ fontFamily: t.type.headingFamily, fontSize: 28, fontWeight: t.type.headingWeight }}>Eval</h1>
        <p style={{ color: t.color.muted, marginTop: t.space.unit * 2 }}>Eval coming soon — quality scoring queue, rating rubric, call review.</p>
      </main>
    </div>
  )
}
